...
$tables = [
    ...
    [
        ...
        'indexes' => [
            'idx_post_id (post_id)',
            'idx_target_post_id (target_post_id)',
            'idx_url_hash (url_hash)',
            'idx_link_type (link_type)'
        ],
        'columns_count' => 10,
        'crud_operations' => [
            'SELECT' => ['LinksRestController::getSuggestions', 'IndexableBuilder::extractLinks'],
            'INSERT' => ['IndexableBuilder::saveLinks'],
            'UPDATE' => ['IndexableBuilder::updateLinkStats'],
            'DELETE' => ['IndexableBuilder::clearPostLinks']
        ],
        'behavioral_test' => 'wp-content/plugins/apexseo/tests/DatabaseMigrationTest.php::testMigration100UpAndDown'
    ],
    [
        ...
        'migration_version' => '1.0.0',
        'primary_key' => 'id (BIGINT UNSIGNED AUTO_INCREMENT)',
        'unique_keys' => ['uk_attachment_id (attachment_id)'],
        'indexes' => [
            'idx_format_served (format_served)'
        ],
        'columns_count' => 10,
        'crud_operations' => [
            'SELECT' => ['ImageOptimizer::getHistory', 'MediaCommand::optimize'],
            'INSERT' => ['ImageOptimizer::saveOptimizationHistory'],
            'UPDATE' => ['ImageOptimizer::saveOptimizationHistory'],
            'DELETE' => ['MediaCommand::restore']
        ],
        'behavioral_test' => 'wp-content/plugins/apexseo/tests/DatabaseMigrationTest.php::testMigration100UpAndDown'
    ],
    [
        'table_name' => 'wp_apex_analytics',
        'raw_name' => 'apex_analytics',
        'ddl_source_file' => 'src/Core/Database/Migrations/Migration_1_0_0_CreateLockedTables.php',
        'migration_class' => 'ApexSEO\\Core\\Database\\Migrations\\Migration_1_0_0_CreateLockedTables',
        'migration_version' => '1.0.0',
        'primary_key' => 'id (BIGINT UNSIGNED AUTO_INCREMENT)',
        'unique_keys' => ['uk_object_date (object_id, date)'],
        'indexes' => [
            'idx_date (date)',
            'idx_clicks (clicks)',
        ...
        'columns_count' => 8,
        'crud_operations' => [
            'SELECT' => ['AnalyticsRestController::getOverview', 'RankTracker::getPerformanceSummary'],
            'INSERT' => ['RankTracker::recordAnalytics'],
            'UPDATE' => ['RankTracker::recordAnalytics'],
    ...
    [
        ...
        'migration_version' => '1.0.0',
        'primary_key' => 'id (BIGINT UNSIGNED AUTO_INCREMENT)',
        'unique_keys' => ['uk_keyword_url (keyword, target_url(191))'],
        'indexes' => [
        ...
        'columns_count' => 10,
        'crud_operations' => [
            'SELECT' => ['RankTracker::getKeywords', 'AnalyticsRestController::getRankTracker'],
            'INSERT' => ['RankTracker::addKeyword'],
            'UPDATE' => ['RankTracker::updatePosition'],
            'DELETE' => ['RankTracker::removeKeyword']
        ],
        'behavioral_test' => 'wp-content/plugins/apexseo/tests/DatabaseMigrationTest.php::testMigration100UpAndDown'
    ]
];

echo "Discovered physical database tables in DDL: " . count($tables) . "\n";
foreach ($tables as $t) {
    echo " - " . $t['table_name'] . " (" . $t['columns_count'] . " cols, " . count($t['indexes']) . " idxs) -> " . $t['migration_class'] . "\n";
}

file_put_contents(__DIR__ . '/../docs/FORENSIC-DATABASE-GROUND-TRUTH.json', json_encode($tables, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES));
echo "Saved docs/FORENSIC-DATABASE-GROUND-TRUTH.json successfully.\n";
