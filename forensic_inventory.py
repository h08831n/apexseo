...
def analyze_php_files():
    ...
    for c in classes:
        print(f"  Class: {c[0]} in {c[1]}")
    print(f"\nAbstract Classes: {len(abstract_classes)}")
    for ac in abstract_classes:
        print(f"  Abstract: {ac[0]} in {ac[1]}")
    print(f"\nInterfaces: {len(interfaces)}")
    for i in interfaces:
        print(f"  Interface: {i[0]} in {i[1]}")
    print(f"\nTODO/FIXME count: {len(todos)}")
    for t in todos:
        print(f"  {t[0]}:{t[1]} -> {t[2]}")
    print(f"\nEmpty methods count: {len(empty_methods)}")
